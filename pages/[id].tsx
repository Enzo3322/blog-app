...
export default function Post() {
    ...
    const { data, error, isLoading } = useQuery('getPost', () => getPostById({ id }), { enabled: !!id, retry: false })

    const Template = ({ children }: any) => {
        return (
            <main
                className={`bg- flex min-h-screen flex-col items-center justify-between md:p-24 p-5 pt-24 ${isDarkTheme ? darkThemeClasses : whiteThemeClasses
                    } ${inter.className}`}
            >
                <button
